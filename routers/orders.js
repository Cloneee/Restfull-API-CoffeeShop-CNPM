    ...
    Promise.all([
      ...
      .then(([data, total]) =>
        res.json({
          ...
          limit: limit,
          totalPage:
            total % limit != 0 ? parseInt(total / limit + 1) : total / limit,
          data: data,
        })
      )
      .catch((err) =>
...
router.get("/customer/:customerId", async function (req, res) {
  ...
  Promise.all([
    Order.find({ customer: req.params.customerId })
      ...
      .sort({ dateOrdered: -1 })
      .select("-__v"),
    Order.count({ customer: req.params.customerId }),
  ])
    .then(([data, total]) =>
      res.json({
        currentPage: page,
...
router.post("/toggle", (req, res) => {
  Order.findByIdAndUpdate(req.body._id, { status: req.body.status })
    .then(() =>
      res.json({ msg: "Update status success", status: req.body.status })
    )
...
router
  ...
  .get(async (req, res) => {
    const orders = await Order.find({
      customer: req.locals.payload.customerId,
    });
