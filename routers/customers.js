...
router
  ...
  .post(function (req, res) {
    ...
    newCustomer.save(function (err) {
      if (!err) {
        return res.status(200).send("Successfully added a new Customer.");
      } else {
    ...
    Customer.findOneAndUpdate(
      { _id: req.params.customerId },
      {
        ...
        password: req.body.password
          ? bcrypt.hashSync(req.body.password, parseInt(process.env.SALT_ROUND))
      ...
      function (err) {
        if (!err) {
          res.status(200).send("Successfully updated the selected Customer.");
        } else {
          res.status(404).json({
            success: false,
            error: err.message,
